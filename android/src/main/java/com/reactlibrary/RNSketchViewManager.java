...
public class RNSketchViewManager extends SimpleViewManager<SketchViewContainer> {
  ...
  private static final int COMMAND_CHANGE_COLOR = 777;
   private static final int COMMAND_CHANGE_THICKNESS = 999;
  ...
  public void receiveCommand(SketchViewContainer root, int commandId, @Nullable ReadableArray args) {
    ...
    switch (commandId) {
      case COMMAND_CHANGE_THICKNESS:
        Assertions.assertNotNull(args);
        String thicknessSize = args.getString(0);
        root.sketchView.setToolThickness(Float.parseFloat(thicknessSize));
        return;
      case COMMAND_CHANGE_COLOR:
        Assertions.assertNotNull(args);
        String rgbColor = args.getString(0);
        root.sketchView.changeColor(rgbColor);
        return;
